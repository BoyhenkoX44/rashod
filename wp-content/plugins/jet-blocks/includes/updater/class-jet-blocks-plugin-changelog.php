 ...
 * @since 1.0.0
 */
class Jet_Blocks_Plugin_Changelog {
	...
	public function plugin_row_meta( $plugin_meta, $plugin_file, $plugin_data ) {
		...
		if ( $plugin === $plugin_file && empty( $plugin_data['update'] ) ) {
			...
			$plugin_meta['view-details'] = sprintf( '<a href="%s" class="thickbox open-plugin-details-modal" aria-label="%s" data-title="%s">%s</a>',
				...
				esc_attr( sprintf( __( 'More information about %s', 'jet-blocks' ), $this->api['name'] ) ),
				esc_attr( $this->api['name'] ),
				esc_html__( 'View details', 'jet-blocks' )
			);
...
if ( ! function_exists( 'jet_blocks_plugin_changelog' ) ) {
	function jet_blocks_plugin_changelog() {
		return new Jet_Blocks_Plugin_Changelog();
	}
}
