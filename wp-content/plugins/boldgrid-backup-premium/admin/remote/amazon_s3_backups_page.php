...
class Boldgrid_Backup_Premium_Admin_Remote_Amazon_S3_Backups_Page {
	...
	public function admin_enqueue_scripts( $hook ) {
		if ( 'boldgrid-backup_page_boldgrid-backup' !== $hook ) {
			return;
		}
	...
	public function wp_ajax_download() {
		...
		$result = $this->premium_core->amazon_s3->download( null, $key );
		...
		if ( $result ) {
			$this->core->notice->add_user_notice(
				sprintf(
					...
					/* 1 */ $key,
					/* 2 */ __( 'BoldGrid Backup Premium - Amazon S3 Download', 'boldgrid-backup' )
				),
