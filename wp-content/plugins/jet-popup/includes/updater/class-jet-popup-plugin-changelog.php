 ...
 * @since 1.0.0
 */
class Jet_Popup_Plugin_Changelog {
	...
	public function plugin_row_meta( $plugin_meta, $plugin_file, $plugin_data ) {
		...
		if ( $plugin === $plugin_file && empty( $plugin_data['update'] ) ) {
			...
			$plugin_meta['view-details'] = sprintf( '<a href="%s" class="thickbox open-plugin-details-modal" aria-label="%s" data-title="%s">%s</a>',
				...
				esc_attr( sprintf( __( 'More information about %s', 'jet-popup' ), $this->api['name'] ) ),
				esc_attr( $this->api['name'] ),
				esc_html__( 'View details', 'jet-popup' )
			);
...
if ( ! function_exists( 'jet_popup_plugin_changelog' ) ) {
	function jet_popup_plugin_changelog() {
		return new Jet_Popup_Plugin_Changelog();
	}
}
